...
public abstract class AbstractTemplateEngine {
    ...
    public AbstractTemplateEngine batchOutput() {
        ...
        // MapperFactory.java
        /*if (tableInfoList.size() > 0 && pathInfo.getMapper() != null) {
            String mapperFile = (pathInfo.getMapper() + File.separator + ConstVal.MAPPER_FACTORY_NAME + suffixJava());
            if (isCreate(mapperFile)) {
                params.put("mapperFactoryName", ConstVal.MAPPER_FACTORY_NAME);
                writer(params, templateFilePath(template.getMapperFactory()), mapperFile);
            }
        }*/
        // Page.java和PageResult.java
       /* if (createPage) {
            String mapperFile = (pathInfo.getPage() + File.separator + ConstVal.PAGE_NAME + suffixJava());
            writer(params, templateFilePath(template.getPage()), mapperFile);
            mapperFile = pathInfo.getPageResult() + File.separator + ConstVal.PAGE_RESULT_NAME + suffixJava();
            writer(params, templateFilePath(template.getPageResult()), mapperFile);
        }*/
        // Ajax.java
       /* if (createAjax) {
            String ajaxFile = (pathInfo.getAjax() + File.separator + ConstVal.AJAX_NAME + suffixJava());
            if (isCreate(ajaxFile)) {
                writer(params, templateFilePath(template.getAjax()), ajaxFile);
            }
        }*/
        return this;
    }
